...
use optimistic_oracle::contracts::{
    finder::finder, interfaces::{IFinderDispatcher, IFinderDispatcherTrait}
};
...
fn test_finder_change_implementation_address() {
    ...
    let expected_event = finder::Event::InterfaceImplementationChanged(
        finder::InterfaceImplementationChanged {
            interface_name: interface_name, new_implementation_address: new_implementation_address
        }
    );
