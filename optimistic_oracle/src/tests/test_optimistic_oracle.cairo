...
use optimistic_oracle::tests::setup::{OWNER,DISPUTER,INITIAL_SUPPLY,oo_full_config, setup_optimistic_oracle,setup_identifier_whitelist,setup_finder, setup_address_whitelist,setup_store, setup_mock_erc20, FINAL_FEE, setup_mock_oracle_ancillary};
use optimistic_oracle::contracts::{
    interfaces::{IOptimisticOracleDispatcher, IOptimisticOracleDispatcherTrait, IStoreDispatcherTrait,IFinderDispatcherTrait, IIdentifierWhitelistDispatcherTrait, IAddressWhitelistDispatcherTrait,IMockOracleAncillaryConfigurationDispatcherTrait }
};
...
fn test_oo_assert_truth_with_default(){
    ...
    start_warp(CheatTarget::One(oo.contract_address), starknet::get_block_timestamp() + liveness-1);
    ...
    oracle_config.push_price('ASSERT_TRUTH', time.into(),oo.stamp_assertion(assertion_id),1000000000000000000 );
